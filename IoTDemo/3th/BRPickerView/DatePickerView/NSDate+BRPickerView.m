...
+ (nullable NSDate *)br_setChaWithNowYear:(NSInteger)year{
    return [self br_setYear: [[NSDate date] br_year] + year month:[[NSDate date] br_month] day:[[NSDate date] br_day] hour:-1 minute:-1 second:-1];
}
...
+ (NSDate *)br_getDate:(NSString *)dateString format:(NSString *)format {
    ...
    // 防止12小时制的日期出问题
    if (!destDate) {
        NSString *localeID = [Localisator isChinese] ? @"zh_CN" : @"en_US";
        
        dateFormatter.locale = [[NSLocale alloc] initWithLocaleIdentifier:localeID];
...
/** 获取 日期加上/减去多少小时数后的新日期 */
-(NSDate *)br_getNewDateFromNowAddHour:(NSInteger)hour{
    // hour 为正数时，表示几小时之后的日期；负数表示几小时之前的日期
