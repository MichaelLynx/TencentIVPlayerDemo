...
-(NSAttributedString*)attributedStringWithStyleBook:(NSDictionary*)fontbook
{
     ...
     * 处理大于号小于号等特殊字符
     */
    NSRange ltrang = [ms rangeOfString:@"&lt;"];
    if (ltrang.length != 0) {
        [as replaceCharactersInRange:ltrang withString:@">"];
    }
    NSRange gtrang = [ms rangeOfString:@"&gt;"];
    if (gtrang.length != 0) {
       [as replaceCharactersInRange:gtrang withString:@"<"];
    }
