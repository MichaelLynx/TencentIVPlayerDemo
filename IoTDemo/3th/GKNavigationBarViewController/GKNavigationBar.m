        ...
        for (UIView *subview in self.subviews) {
            if ([NSStringFromClass(subview.class) containsString:@"ContentView"]) {
                ...
                if ([UIDevice currentDevice].systemVersion.floatValue >= 13.0) {
                             ...
                             subview.frame = CGRectMake(-margins.left+subview.frame.origin.x, -margins.top+subview.frame.origin.y, margins.left + margins.right + subview.frame.size.width, margins.top + margins.bottom + subview.frame.size.height);
                         } else {
...
- (UIView*)hitTest:(CGPoint)point withEvent:(UIEvent *)event{
    ...
    if( self.gk_navBarBackAction &&(hitView == self || hitView.superview == self)){
        return nil;
    }
