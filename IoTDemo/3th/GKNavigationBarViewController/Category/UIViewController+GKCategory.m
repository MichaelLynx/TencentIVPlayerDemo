...
static const void* GKNavBackActionKey     = @"GKNavBackActionKey";
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (@available(iOS 13.0, *)) {
          gk_swizzled_method(class, @selector(presentViewController:animated:completion:), @selector(gk_presentViewController:animated:completion:));
        }
    });
}
-(void)gk_presentViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion{
    ...
    NSLog(@"========  %@",@(viewControllerToPresent.modalPresentationStyle));
    
    
    if (![viewControllerToPresent isKindOfClass:NSClassFromString(@"MineViewController")]) {
        if (viewControllerToPresent.modalPresentationStyle<2) {
...
-(BOOL)gk_navBarBackAction{
    return [objc_getAssociatedObject(self, GKNavBackActionKey) boolValue];
}
-(void)setGk_navBarBackAction:(BOOL)gk_navBarBackAction{
    objc_setAssociatedObject(self, GKNavBarAlphaKey, @(gk_navBarBackAction), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    
    if ([self isKindOfClass:[GKNavigationBarViewController class]]) {
        GKNavigationBarViewController *vc = (GKNavigationBarViewController *)self;
        
        vc.gk_navigationBar.gk_navBarBackAction = gk_navBarBackAction;
    }
