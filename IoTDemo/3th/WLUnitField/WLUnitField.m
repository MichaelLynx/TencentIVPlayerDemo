...
- (void)initialize {
    ...
    _trackTintBackgroundColor =_trackTintBackgroundColor?: [UIColor clearColor];
...
- (void)drawRect:(CGRect)rect {
   ...
   [self _drawTrackBackground:rect unitSize:unitSize];
...
/**
 * 绘制跟踪框背景，如果指定的`trackTintBackgroundColor`为 nil 则不绘制
 @param rect 控件绘制的区域
 @param unitSize 单个 input unit 占据的尺寸
 */
- (void)_drawTrackBackground:(CGRect)rect unitSize:(CGSize)unitSize {
    
    [_trackTintBackgroundColor setFill];
...
#pragma mark - 判断左右布局方向
            ...
            for (int i = 0; i <  _characterArray.count; ++i) {
                NSInteger j= i;
                if (isRTL) {
                    j=_inputUnitCount-i-1;
                }
                
                
                
                CGRect unitRect = CGRectMake(j * (unitSize.width + _unitSpace),
                                             0,
                                             unitSize.width,
                                             unitSize.height);
...
- (void)_drawBorder:(CGRect)rect unitSize:(CGSize)unitSize {
    ...
    if (_style == WLUnitFieldStyleBorder) {
        ...
        for (int i = (int)_characterArray.count; i < _inputUnitCount; i++) {
            ...
            CGRect unitLineRect = CGRectMake(j * (unitSize.width + _unitSpace),
...
- (void)_resetCursorStateIfNeeded {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSInteger j= self->_characterArray.count;
        if (isRTL) {
            j=self->_inputUnitCount-self->_characterArray.count-1;
        }
        
        CGRect unitRect = CGRectMake(j * (unitSize.width + self->_unitSpace),
                                     0,
                                     unitSize.width,
                                     unitSize.height);
