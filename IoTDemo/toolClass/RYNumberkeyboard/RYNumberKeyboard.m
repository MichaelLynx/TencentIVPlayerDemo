...
- (IBAction)keyboardViewAction:(UIButton *)sender
{
    ...
    switch (tag)
    {
        ...
        case 1011:
        {
            ...
            if(self.interval && [self.interval integerValue] != 0 && (self.textInput.text.length+1) % ([self.interval integerValue] + 1) == 0)
                [self.textInput insertText:@" "];
        }
            break;
        case 1012:
        ...
        {
            // 数字
            NSString *text = [NSString stringWithFormat:@"%@",@(sender.tag - 1000)];
